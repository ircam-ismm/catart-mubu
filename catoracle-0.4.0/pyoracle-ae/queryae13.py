...
def draw_oracle(index = None):
    ...
    # set only once from ouside
    send_class.send('frgb 255 255 255') # set pen to white
    send_class.send('linesegment %f %f %f %f' % (0, height/2, width, height/2))
    ...
    for i in range(0, N_states, increment):
        ...
        for tran in oracle['trn'][i]:
            ...
            if tran == i + 1:
                ...
                arc_height = (height / 2) + (tran - i) * (width / (2 * height))
                send_class.send('framearc %f %f %f %f %f %f' % (current_x,
            ...
            if oracle['sfx'][i] is not None and oracle['sfx'][i] != 0 and oracle['lrs'][oracle['sfx'][i]] >= lrs_threshold:
                ...
                # draw arc
                arc_height = (height / 2) - (oracle['sfx'][i] - i) * (width / (2 * height))
    ...
    for i in range(0, N_states, increment):
        ...
        circle_width = min(5, (0.5 / (N_states / increment) * width) / 2)
...
def draw_next_state(index = None):
    ...
    N_states = len(oracle['sfx'])
    increment = 1
    x_pos = (float(ks[str(index)]) / N_states * width) + 0.5 * 1.0 / N_states * width
    last_x_pos = (float(k_last[str(index)]) / N_states * width) + 0.5 * 1.0 / N_states * width
    # circle_width = (0.5 / (N_states / increment) * width) / 2
    circle_width = min(5, (0.5 / (N_states / increment) * width) / 2)
    send_class.send('paintoval %f %f %f %f %f %f %f' % (last_x_pos - circle_width,
                                                        height/2 - circle_width,
                                                        last_x_pos + circle_width,
                                                        height/2 + circle_width,
                                                        255, 255, 255))
    send_class.send('paintoval %f %f %f %f %f %f %f' % (x_pos - circle_width,
                                                        height/2 - circle_width,
